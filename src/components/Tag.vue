<template>
  <el-badge class="tag" :value="correct_tag.name" :type="correct_tag.type"></el-badge>
</template>
...
export default {
  ...
  computed: {
    correct_tag: function() {
      if (!this.tags) return {name: "das", type: "danger"};
      if (this.tags[0].name == "News") {
        if (!this.tags[1]) return "";
        if (this.tags[1].name == "IF") {
